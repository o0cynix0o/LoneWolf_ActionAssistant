...
- current `main` contains the completed M6 web-GUI migration scaffold under `web/`, with browser-native setup, saves, inventory, combat, stats, campaign, achievements, death recovery, disciplines, modes, combat log, prompt-heavy automation flows, and help surfaces backed by the PowerShell session/HTTP JSON path
...
- the M6 web transition smoke now covers Book Complete -> `continueBook` -> Book `6` to `7` continuation, including discipline, Weaponmastery, safekeeping, starting-gear payloads, and carried-state checks in both PowerShell engines
- the M6 closeout smokes now also cover achievement payloads, prompt-heavy Book `6` / `7` automation, live browser-DOM rendering, and packaged web launchers in both PowerShell engines where applicable
...
1. post-M6 web hardening: live-play polish, long-run browser ergonomics, and non-Windows launch validation when those environments are available
...
- continue post-M6 browser playtesting and polish from the completed M6 scaffold
